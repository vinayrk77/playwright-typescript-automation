...
test("Test to verify the xpath axes", async({page})=>{
    ...
    const rightsibling1: Locator = page.locator("//td[normalize-space()='Maria Anders']/following-sibling::td");
    await expect(rightsibling1).toHaveCount(1);
    ...
    const pre1: Locator = page.locator("//td[text()='Germany']/preceding-sibling::td");
    await expect(pre1).toHaveCount(2);
